...
window.addEventListener("load", function () {
  ...
  if (!product && !cart) {
    ...
    document.getElementById("summaryImgDiv").innerHTML =
  ...
  if (product) {
    ...
    summary.innerHTML += `
      <div class="d-flex align-items-center">
        <div  >
                  <img src='${
                    product.images[0] || "https://placehold.co/500x600/png"
                  }' 
        ...
        <div class="w-100  me-5" >
          ...
          <small class="text-muted " id="Productdetails">${
            product.name
          } / ${productCount} pieces  </small>
        </div>
        <span class=" fw-bold Subtotal text-danger" >$${
    ...
    cart.forEach((product, index) => {
      ...
      <div class="d-flex align-items-center">
        <div>
                  <img src='${
                    product.image || "https://placehold.co/500x600/png"
                  }' 
                  class='card-img-top rounded w-50 summaryImg' 
        ...
        <div class="w-100 me-5" >
          ...
          <small class="text-muted " id="Productdetails">${product.name} / ${
        product.quantity
      } pieces </small>
        </div>
        <span class=" fw-bold Subtotal text-danger" >$${
  ...
  if (productId && productCount) {
    ...
    if (product) {
      const subtotal =
        (product.discountedPrice || product.price) * productCount;
    ...
    if (cartSummary) {
      document.getElementById(
        "Subtotal"
      ).textContent = `$${cartSummary.subtotal.toFixed(2)}`;
      document.getElementById(
        "shipping"
      ).textContent = `$${cartSummary.shipping.toFixed(2)}`;
      document.getElementById(
        "totalPrice"
      ).textContent = `$${cartSummary.total.toFixed(2)}`;
    }
  }
...
document
  .getElementById("saveInformation")
