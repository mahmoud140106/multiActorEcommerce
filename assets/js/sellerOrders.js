...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function loadOrders() {
    ...
    if (!currentUser || currentUser.role !== "seller") {
      showToast("You must be logged in as a Seller to view Orders.", "error");
  ...
  function renderPagination() {
    ...
    const pageCount = Math.ceil(
      (filteredOrders ?? sellerOrders).length / itemsPerPage
    );
  ...
  window.searchOrders = () => {
    ...
    filteredOrders = sellerOrders.filter((o) => {
      ...
      const matchesQuery =
        UserManager.getUserNameById(o.customerId)
          .toLowerCase()
          .includes(query) ||
  ...
  document
    ...
    .addEventListener("change", function (e) {
      if (e.target && e.target.classList.contains("orderNewStatus")) {
        ...
        const index = [...document.querySelectorAll(".orderNewStatus")].indexOf(
          select
        );
        const orderCurrentStatus = document.getElementById(
          `orderCurrentStatus${index}`
        );
